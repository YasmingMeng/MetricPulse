 ...
 * @LastEditTime: 2025-03-09 20:03:10
 */
"use client";
      ...
      <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm w-205 h-115 bg-white shadow-md rounded-md p-6">
        <form className="space-y-6" action="#" method="POST" onSubmit={handleLogin}>
          <div>
            ...
            <div className="mt-2">
              <input type="email" name="email" id="email" autoComplete="email" required className="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-blue-400 hover:outline-blue-300 sm:text-sm/6" />
            </div>
          </div>
          ...
          <div>
            <button type="submit" className="flex w-full justify-center rounded-md bg-blue-400 px-3 py-1.5 text-sm/6 font-semibold text-white shadow-xs hover:bg-blue-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-800">登录</button>
          </div>
        </form>

        <p className="mt-10 text-center text-sm/6 text-gray-500">
         不是会员？
          <a href="#" className="font-semibold text-blue-400 hover:text-blue-800">14天免费试用</a>
        </p>
     </div>
    </div>
  );
